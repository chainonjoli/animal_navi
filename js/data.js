...
// ---- 全60キャラクターデータ（番号順） ----
// キャラ番号(1-60) → 動物・サブタイプの正確なマッピング
// 十二運星: 長生=猿, 沐浴=チータ, 冠帯=黒ひょう, 建禄=ライオン,
//           帝旺=虎, 衰=たぬき, 病=子守熊, 死=ゾウ, 墓=ひつじ,
//           絶=ペガサス, 胎=狼, 養=こじか
//
// 実証済みデータ:
// n=8  → たぬき (磨き上げられたたぬき) - 平野紫耀さん検証済み
// n=12 → ゾウ  (人気者のゾウ)         - 永瀬廉さん検証済み
// n=22 → ペガサス (翼をもつペガサス)   - 高橋海人さん検証済み
// n=42 → チータ (足腰の強いチータ)     - 神宮寺勇太さん検証済み
// n=60 → 虎   (慈悲深い虎)            - 岸優太さん検証済み
...
// ---- 相性チャート（じゃんけんの法則） ----
// MOON > EARTH > SUN > MOON (弱点の方向)
// 同グループ = 最高の仲間
const COMPATIBILITY = {
  same:    { score: 100, label: '最高の仲間✨', desc: '同じ価値観でベストパートナー！' },
  beats:   { score: 80,  label: 'あなたが勝ち🏆', desc: 'あなたが引っ張るリーダー的存在' },
  losesTo: { score: 85,  label: 'ついていきたい💫', desc: '相手から学べることが多い憧れの人' },
};
...
function getCharacterNumber(year, month, day) {
  // 1926年1月1日 を起点として日数を計算
  // 1926-01-01 のキャラ番号はテーブル値から導出（base=26, 0-indexed → +1で1-indexed）
  const base = new Date(1926, 0, 1);
  const target = new Date(year, month - 1, day);
  const diffMs = target.getTime() - base.getTime();
  const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
  // 0-indexed値を取得し、+1で1-60の番号に変換
...
function diagnose(year, month, day) {
  ...
  const charData = CHARACTER_TABLE[charNum - 1]; // 0-indexed
  const profile = ANIMAL_PROFILES[charData.animal];
  return {
    number: charNum,
    ...charData,
    profile,
    group: GROUPS[charData.group],
  };
}
